...
function checkName(){
	...
	if(id_val != ""){
		...
		xmlHttp.send(null);
			...
			nameDiv.innerHTML="<b style='color:red'>用户名不能为空！</b>";
		}
...
function checkEmail(){
	...
	if(em_val != ""){
		...
		xmlHttp.send(null);
			...
			nameDiv.innerHTML="<b style='color:red'>邮箱不能为空！</b>";
		}
...
function checkPw(){
	...
	if(val != ""){
		var len=val.length;
		if(len>18 || len<6 ){
			nameDiv.innerHTML="<b style='color:red'>密码应为6-18个字符！</b>";	
		}else{
				nameDiv.innerHTML="<img src='./images/check_right.gif' />";	
			
		}
	}else{
			nameDiv.innerHTML="<b style='color:red'>密码不能为空！</b>";	
...
function checkRePw(){
	...
	if(re_val != ""){
		if(val == re_val){
				...
				nameDiv.innerHTML="<b style='color:red'>两次输入密码不一致！</b>";
		}

	}else{
			nameDiv.innerHTML="<b style='color:red'>确认密码不能为空！</b>";
	}
}
